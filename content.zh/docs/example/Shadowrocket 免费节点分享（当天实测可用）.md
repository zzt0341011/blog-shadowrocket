---
title: "Shadowrocket 免费节点分享（当天实测可用）"
description: "每日更新的 Shadowrocket 免费节点分享，实测可用。提供免费 SS/SSR/Vmess/Trojan 订阅节点、小火箭导入教程，以及稳定付费机场推荐。"
weight: 1
bookToc: true
bookFlatSection: false
tags: ["Shadowrocket", "免费节点", "小火箭", "免费机场", "科学上网"]
keywords: ["Shadowrocket免费节点", "小火箭免费节点", "免费订阅节点", "免费机场", "shadowrocket节点分享"]
---

# Shadowrocket 免费节点分享（2026年8月6日更新）

> **状态：当天实测可用**　·　**更新频率：每日手动收集更新**

本页每天更新一批从网络公开渠道收集的 Shadowrocket 免费节点，均在**发布当天完成测试**。免费节点适合临时应急、轻度浏览和测速体验；如果你需要长期稳定、看高清视频或工作使用，建议直接看文末的[稳定机场推荐](#稳定机场推荐)。

![shadowrocket-fenxiang-1001.jpg](https://shadowrocket.ink/img/shadowrocket-fenxiang-1001.jpg)

 > **免费节点的特点**：由公网收集、随时可能失效，人数一多就会拥挤、掉线、限速，**不保证稳定性和安全性**。请勿用于登录敏感账号、网银、支付等场景。追求稳定请使用付费机场。


---

## 一、免费节点

下面是今天收集并测试通过的免费节点。你可以**复制单条链接**手动导入，也可以直接使用下方的**订阅链接**一键导入全部节点。





### 节点链接（应急备用）

导入方法见下方[小火箭导入教程](#三shadowrocket-小火箭导入教程)。

```text
hysteria2://sFWVn9pkJpOtvvQX@43.207.185.94:16001?sni=jp-ls-6.nebulacloud.win&insecure=1#hysteria2-743190643
hysteria2://ahGFfsmY8MRbFPYi@57.182.252.243:26001?sni=jp-ls-f-1.nebulacloud.win&insecure=1#hysteria2-738059791
hysteria2://DRJF4I7ft6bkGi1P@43.207.158.159:36001?sni=jp-ls-a6.nebulacloud.win&insecure=1#hysteria2-738059861
hysteria2://yQVXqI1PLqYlv9CJ@3.113.215.96:16001?sni=jp-ls-h-3.nebulacloud.win&insecure=1#hysteria2-743190631
hysteria2://lNHF8jiIOvOjFB84@35.74.66.136:46001?sni=jp-ls-g-3.nebulacloud.win&insecure=1#hysteria2-738059781
hysteria2://VpfnFCX9SXb1wuk9@35.74.27.195:36001?sni=jp-ls-d-5.nebulacloud.win&insecure=1#hysteria2-738059889
trojan://EQ30679154@united-frog.rooster465.autos:443?security=tls&sni=united-frog.rooster465.autos&type=tcp&headerType=none#trojan-710529857
trojan://EQ30679154@curious-ocelot.rooster465.autos:443?security=tls&sni=curious-ocelot.rooster465.autos&type=tcp&headerType=none#trojan-710529859
hysteria2://IVhmHXmvVAKUiwhE@13.214.127.179:36001?sni=sg-ls-q-2.nebulacloud.win&insecure=1#hysteria2-743190677
trojan://EQ30679154@willing-eagle.rooster465.autos:443?security=tls&sni=willing-eagle.rooster465.autos&type=tcp&headerType=none#trojan-733881338
vmess:

```



**测速小贴士**：导入后长按节点可测延迟，优先选择延迟低（绿色）的节点。如果全部无法连接，说明当日节点已失效，请等待明日更新或改用付费机场。


---

## 二、什么是 Shadowrocket（小火箭）

[Shadowrocket](https://shadowrocket.ink/zh) 俗称「小火箭」，是 iOS / iPadOS 上一款广受欢迎的代理客户端，支持 Shadowsocks(SS)、ShadowsocksR(SSR)、VMess、VLESS、Trojan 等主流协议，可以通过导入节点或订阅链接实现科学上网。

![shadowrocket-fenxiang-1003.jpg](https://shadowrocket.ink/img/shadowrocket-fenxiang-1003.jpg)

它需要在**非中国区 Apple ID** 的 App Store 中购买下载（约 2.99 美元）。安卓用户可使用 v2rayNG、Clash 等替代客户端，本站节点同样适用。

---

## 三、Shadowrocket（小火箭）导入教程


**单条链接手动导入**

1. 复制上方任意一条 `ss://` / `vmess://` / `trojan://` 链接；
2. 回到 Shadowrocket，首页顶部会自动弹出「添加节点」提示，点击确认即可；
3. 选中该节点，把首页顶部开关拨到「打开」，即完成连接。

**连接建议**：路由模式一般选择「配置」或「规则判定」，实现国内直连、国外走代理，速度更快也更省流量。

![shadowrocket-fenxiang-1002.jpg](https://shadowrocket.ink/img/shadowrocket-fenxiang-1002.jpg)

---

## 四、免费节点 vs 付费机场，怎么选？

| 对比项 | 免费节点 | 付费机场 |
| --- | --- | --- |
| 稳定性 | 随时可能失效、掉线 | 稳定，SLA 保障 |
| 速度 | 人多拥挤、限速明显 | 独享/优化带宽，速度快 |
| 高清视频 | 基本无法流畅观看 | 4K / Netflix / YouTube 无压力 |
| 安全性 | 来源不明，有风险 | 正规运营，隐私更有保障 |
| 节点数量 | 少且不固定 | 多地区、多线路可选 |
| 适用场景 | 临时应急、测速体验 | 长期使用、工作、追剧 |

一句话总结：**免费节点用来「应急」，付费机场用来「日常」。** 如果你每天都要用、且看重速度和稳定，付费机场的性价比其实非常高。

---

## 五、稳定机场推荐 {#稳定机场推荐}

以下是我长期使用、综合体验较好的几家机场。挑选机场时主要看四点：**节点速度、稳定性、解锁能力（Netflix/ChatGPT/流媒体）、售后服务**。


> 以下链接为本站推广链接，通过它们注册对你没有任何额外费用，还能支持本站每日更新，感谢支持 🙏


### 🥇 网际快车机场（综合首选）

- **适合人群**：追求速度和稳定的日常用户
- **特点**：（IEPL/IPLC 专线、解锁流媒体、不限速）
- **套餐**：（7元20G套餐不限时）
- 👉 **[点击注册 · 新用户优惠](https://1.jnk.ink/ad2RVl)**

### 🥈 魔戒机场（高性价比）

- **适合人群**：预算有限、轻中度使用
- **特点**：（价格便宜、按量付费、节点够用）
- **套餐**：（1元1G不限时）
- 👉 **[点击注册](https://1.jnk.ink/L4q20S)**

### 🥉 happy猫（大流量/家庭共享）

- **适合人群**：多人共享、大流量下载
- **特点**：（大流量、多设备、家庭套餐）
- **套餐**：（20元200G不限时）
- 👉 **[点击注册](https://1.jnk.ink/5KiTxY)**

---

* 以下机场按照流量付费，网站里有软件的使用和安装教程
* 购买流量以后，不限制时间，流量用完为止
* 如果网站无法访问，则说明已经被墙，更换其他网站即可

| 名 称 | 价 格 | 流 量 | 节点数 |
| :--- | :--- | :--- | :--- |
| [魔戒](https://1.jnk.ink/L4q20S) | 1元 | 1G | 30个 |
| [网际快车](https://1.jnk.ink/ad2RVl) | 7元 | 20G | 54个 |
| [牛逼](https://1.jnk.ink/LYet7x) | 14元 | 200G | 31个 |
| [飞鸟](https://1.jnk.ink/i7OhaC) | 10元 | 200G | 25个 |
| [皮卡丘](https://1.jnk.ink/d07dCA) | 15元 | 20G | 40个 |
| [happy猫](https://1.jnk.ink/5KiTxY) | 20元 | 200G | 27个 |
| [农夫山泉](https://1.jnk.ink/i1fXTMYk)  | 45元   | 200G |40个|
| [宝贝云](https://1.jnk.ink/xxPwfy) | 55元 | 600G | 64个 |
| [自由猫](https://1.jnk.ink/haO8Dr) | 89元 | 200G | 71个 |
| [飞兔](https://1.jnk.ink/bbXkiN) | 30元 | 100G | 80个 |

## 六、常见问题（FAQ）

**Q：为什么今天的免费节点连不上了？**
A：免费节点由公网收集，随时可能失效或被封。请下拉刷新订阅，或等待本站次日更新；追求稳定建议使用付费机场。

**Q：免费节点安全吗？能登录账号吗？**
A：免费节点来源不明，**不建议**用于登录邮箱、社交、网银、支付等敏感操作。重要场景请使用正规付费机场。

**Q：安卓 / Windows / Mac 能用这些节点吗？**
A：可以。这些节点是通用协议，安卓用 v2rayNG 或 Clash，电脑端用 Clash 系客户端导入订阅即可。

**Q：Shadowrocket 在哪里下载？**
A：需在非中国区 App Store 购买下载，价格约 2.99 美元。安卓用户可用免费替代客户端。

**Q：节点多久更新一次？**
A：本站**每天手动收集并测试**后更新，建议收藏本页（Ctrl+D）每日查看最新节点。

---

## 七、免责声明

本站分享的免费节点均来源于**互联网公开渠道**，仅供学习和技术研究使用，用于测试网络连通性与客户端配置。请在遵守所在国家/地区法律法规的前提下使用，因使用本站信息产生的任何后果由使用者自行承担，本站不承担任何责任。请勿将节点用于任何非法用途。

---

> 📌 **收藏本页，每日更新。** 想要长期稳定的科学上网体验，欢迎参考文末的[机场推荐](#稳定机场推荐)。
